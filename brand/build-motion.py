...
DX, DY = sx - ex, sy - ey
CSS = f"""/* The tile arrives: it starts in its notch, square, and comes off the block.
   Generated by brand/build-motion.py. The numbers are the mark's own geometry. */
.logo .tile{{
  transform-box:fill-box; transform-origin:center;
  animation:otb-off {motion.DURATION}s cubic-bezier({','.join(f'{v:g}' for v in motion.EASE)}) both;
}}
@keyframes otb-off{{
  from{{transform:translate({DX:.3f}px,{DY:.3f}px) rotate({-deg:g}deg)}}
  to{{transform:none}}
}}
...
need = [f"translate({DX:.3f}px,{DY:.3f}px)", "@keyframes otb-off", 'class="tile"']
